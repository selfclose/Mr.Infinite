...
class ModelController_old extends UtilitiesController
{
    protected $id;
    protected $table;
    public $dataModel;
    ...
    public function getTable()
    {
        ...
        if (is_null($this->table)) {
            ...
            return $this->table = strtolower($ex[count($ex) - 1]);
        }
        return $this->table;
    }
...
//        }
    ...
    static function setup($host = 'localhost', $db_name, $username, $password, $checkIsConnect = true, $debug)
    {
        \R::setup("mysql:host={$host};dbname={$db_name}", $username, $password);

//If connected
        if ($checkIsConnect && \R::testConnection()) {
        } else {
